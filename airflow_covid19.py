...
def save_data_into_db(**kwargs):
    ...
    print(data)
    for r in data:
        Txn_date_1 = datetime.strptime(r["update_date"],'%Y-%m-%d %H:%M:%S') + timedelta(days=1)
        Txn_date = Txn_date_1.strftime('%Y-%m-%d')
        New_case = r["new_case"]
...
with DAG('covid19_data_pipeline',
    ...
    t1 = SimpleHttpOperator(
        ...
        response_filter=lambda response: json.dumps(json.loads(response.text)),
        dag=dag
    )
